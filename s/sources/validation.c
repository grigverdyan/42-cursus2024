...
int	check_args(int argc, char **argv)
{
	...
	if (nums_count == 0)
		error_message("[ARGUMENT ERROR]: Numbers must be inserted!\n");
	return (nums_count);
}
	...
	*nums_count += 1;
	if (ft_isdigit(num[0]) != 1 && num[0] != '+' && num[0] != '-')
	{
		free_and_exit(temp, get_matrix_size(temp),
			"[ARGUMENT ERROR]: Inserted invalid character!\n");
	}
	while (num[++i])
	{
		...
		{
			free_and_exit(temp, get_matrix_size(temp),
				"[ARGUMENT ERROR]: Inserted invalid charecter!\n");
		}
	}
	check_num1(num, temp);
}
...
void	check_num1(char *num, char **temp)
{
	...
	if ((num[0] == '+' || num[0] == '-') && num[1] == '0')
		free_and_exit(temp, size, "[ARGUMENTS ERROR]\n");
